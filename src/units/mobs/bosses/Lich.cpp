...
void Lich::Attack(Unit& target, ActionCompletionCallback callback)
{
...
void Lich::PerformAttack(AnimatedUnit& target, ActionCompletionCallback callback)
{
    if (!m_active || m_currentHealth <= 0 || !target.IsActive())
    {
        if (callback)
...
void Lich::UseSkill(Unit& target, ActionCompletionCallback callback)
{
