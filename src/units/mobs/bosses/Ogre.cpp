...
void Ogre::Attack(Unit& target, ActionCompletionCallback callback)
{
...
void Ogre::PerformAttack(AnimatedUnit& target, ActionCompletionCallback callback)
{
    if (!m_active || m_currentHealth <= 0 || !target.IsActive())
    {
        if (callback)
...
void Ogre::UseSkill(Unit& target, ActionCompletionCallback callback)
{
