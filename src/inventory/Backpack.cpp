...
Backpack::Backpack(std::vector<std::vector<std::pair<Item, int>>> initial) {
    backpack.resize(backpackRows, std::vector<std::pair<Item,int>>(backpackCols, { Item(), 0 }));
    for (int i = 0; i < std::min(backpackRows, (int)initial.size()); ++i) {
        for (int j = 0; j < std::min(backpackCols, (int)initial[i].size()); ++j) {
            backpack[i][j] = initial[i][j];
        }
    }
}
