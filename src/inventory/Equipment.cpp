...
Equipment::Equipment(ItemManager& itemManager, const std::string& weaponID, const std::string& headArmorID, 
                    const std::string& bodyArmorID, const std::string& footArmorID, 
                    const std::string& pendantID) : Equipment() {
...
bool Equipment::unequipItemToBackpack(Backpack& backpack, int x, int y, const std::string& slotType) {
    try {
        ...
        if (slotType == "Weapon") {
            ...
            if (itemToUnequip.isNull()) return false;
        } else if (slotType == "HeadArmor") {
