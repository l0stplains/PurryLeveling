...
QuestInfo::QuestInfo(GameContext& gameContext, float windowWidth, float windowHeight)
    : m_windowWidth(windowWidth), m_windowHeight(windowHeight), m_gameContext(gameContext)
{
...
void QuestInfo::render(const Dungeon& dungeon)
{
    ...
    ImVec2 windowPos(viewport->WorkPos.x + viewport->WorkSize.x - m_windowWidth - 10,  // Upper
                                                                                       // right
    ...
    if (ImGui::Begin("Quest Info", nullptr, flags))
    {
        ...
        {
            // Quest Type
            std::string questTypeStr = (quest.getType() == QuestType::KILL) ? "Kill Mobs"
                                                                            : "Deal Damage";
            ImGui::Text("Type: %s", questTypeStr.c_str());

            // Progress Bar
            float progressRatio = quest.getTargetCount() > 0
                                      ? static_cast<float>(quest.getCurrentProgress()) /
                                            static_cast<float>(quest.getTargetCount())
                                      : 0.0f;
