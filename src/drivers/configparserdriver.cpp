...
int main() {
    ...
    tests.push_back({"BackpackConfig","data/configtest/backpack.txt", std::make_unique<BackpackConfigParser>()});
    tests.push_back({"MobLootConfig", "data/configtest/mobloot.txt",   std::make_unique<MobLootConfigParser>()});
    tests.push_back({"InventoryConfig","data/configtest/equipment.txt", std::make_unique<InventoryConfigParser>()});

    for (auto &tc : tests) {
        std::cout << "--- Testing " << tc.name << " ---\n";
        if (!tc.parser->ParseFromFile(tc.filename)) {
            std::cerr << "ERROR parsing " << tc.filename << ": "
                      << tc.parser->GetLastError() << "\n";
            continue;
        }
