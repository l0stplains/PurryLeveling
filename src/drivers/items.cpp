...
void printBackpack(const Backpack& backpack, int rows = 8, int cols = 4) {
...
void printEquippedItems(const Equipment& equipment) {
    ...
    std::cout << "Armor (Kepala): " << (headArmor.isNull() ? "<Empty>" : headArmor.getName()) << std::endl;
    std::cout << "Armor (Badan) : " << (bodyArmor.isNull() ? "<Empty>" : bodyArmor.getName()) << std::endl;
    std::cout << "Armor (Kaki)  : " << (footArmor.isNull() ? "<Empty>" : footArmor.getName()) << std::endl;
    std::cout << "Pendant       : " << (pendant.isNull() ? "<Empty>" : pendant.getName()) << std::endl;
}
...
int main() {
    try {
        ...
        std::vector<std::vector<std::pair<Item, int>>> initialBackpack(8, std::vector<std::pair<Item, int>>(4, {Item(), 0}));
        
        // Add some initial items to specific positions
        initialBackpack[1][0] = {itemManager.getItem("HTL"), 2}; // Heavy Helmet at position (1,A)
        initialBackpack[2][0] = {itemManager.getItem("BAC"), 64}; // Battle Armor at position (2,A)
        initialBackpack[3][0] = {itemManager.getItem("BAC"), 24}; // Battle Armor at position (3,A)
        ...
        try {
            ...
            if (findEmptySlot(backpack, emptyRow, emptyCol)) {
                std::cout << "Adding 1 Longbow to position (" << emptyRow << "," << (char)('A'+emptyCol) << ")..." << std::endl;
            ...
            bool foundBow = false;
            for (int i = 0; i < 8 && !foundBow; ++i) {
                for (int j = 0; j < 4 && !foundBow; ++j) {
                    try {
                        Item item = backpack.getItemAtTile(i, j);
            ...
            if (foundBow) {
                std::cout << "Adding another Longbow to position (" << bowRow << "," << (char)('A'+bowCol) << ")..." << std::endl;
            ...
            try {
                ...
                for (int i = 0; i < 8; ++i) {
                    ...
                    if (nonSwordRow != -1) break;
                }
                
                if (nonSwordRow != -1) {
                    std::cout << "Trying to add a Sword to position (" << nonSwordRow << "," << (char)('A'+nonSwordCol) << ")..." << std::endl;
        ...
        try {
            ...
            for (int i = 0; i < 8 && !foundHelmet; ++i) {
                for (int j = 0; j < 4 && !foundHelmet; ++j) {
                    try {
                        Item item = backpack.getItemAtTile(i, j);
            ...
            if (foundHelmet) {
                std::cout << "Taking 1 item from position (" << helmetRow << "," << (char)('A'+helmetCol) << ")..." << std::endl;
        ...
        try {
            ...
            if (armorRow != -1 && emptyRow != -1) {
                std::cout << "Moving Battle Armor from (" << armorRow << "," << (char)('A'+armorCol) 
                          << ") to (" << emptyRow << "," << (char)('A'+emptyCol) << ")..." << std::endl;
            ...
            if (!hasBreastplate) {
                ...
                if (findEmptySlot(backpack, breastplateRow, breastplateCol)) {
                    ...
                    backpack.addItemAtTile(breastplateRow, breastplateCol, itemManager.getItem("BPA"), 1);
            ...
            if (findEmptySlot(backpack, emptyRow2, emptyCol2) && (breastplateRow != -1)) {
                std::cout << "Moving Breastplate from (" << breastplateRow << "," << (char)('A'+breastplateCol) 
                          << ") to (" << emptyRow2 << "," << (char)('A'+emptyCol2) << ")..." << std::endl;
            ...
            if (chainMailRow != -1 && leatherRow != -1) {
                std::cout << "Swapping Chain Mail at (" << chainMailRow << "," << (char)('A'+chainMailCol) 
                          << ") with Leather Armor at (" << leatherRow << "," << (char)('A'+leatherCol) << ")..." << std::endl;
            ...
            if (sword1Row != -1) {
                ...
                std::cout << "Added 1 sword to position (" << sword1Row << "," << (char)('A'+sword1Col) << ")" << std::endl;
            }
            
            findEmptySlot(backpack, sword2Row, sword2Col);
            if (sword2Row != -1) {
                backpack.addItemAtTile(sword2Row, sword2Col, itemManager.getItem("SWD"), 1);
                std::cout << "Added 1 sword to position (" << sword2Row << "," << (char)('A'+sword2Col) << ")" << std::endl;
                ...
                std::cout << "Moving sword from (" << sword1Row << "," << (char)('A'+sword1Col) 
                          << ") to (" << sword2Row << "," << (char)('A'+sword2Col) << ")..." << std::endl;
        ...
        try {
            ...
            if (findEmptySlot(backpack, bootsRow, bootsCol)) {
                std::cout << "Adding boots to position (" << bootsRow << "," << (char)('A'+bootsCol) << ")..." << std::endl;
            ...
            if (findEmptySlot(backpack, swordRow, swordCol)) {
                std::cout << "Adding sword to position (" << swordRow << "," << (char)('A'+swordCol) << ")..." << std::endl;
            ...
            if (findEmptySlot(backpack, helmetRow, helmetCol)) {
                std::cout << "Adding helmet to position (" << helmetRow << "," << (char)('A'+helmetCol) << ")..." << std::endl;
            ...
            if (findEmptySlot(backpack, pendantRow, pendantCol)) {
                std::cout << "Adding pendant to position (" << pendantRow << "," << (char)('A'+pendantCol) << ")..." << std::endl;
            ...
            if (findItemTypeSlot(backpack, "Weapon", weaponRow, weaponCol)) {
                std::cout << "Equipping weapon from position (" << weaponRow << "," << (char)('A'+weaponCol) << ")..." << std::endl;
                bool weaponEquipped = equipment.equipItemFromBackpack(backpack, weaponRow, weaponCol, "Weapon");
                std::cout << "Weapon " << (weaponEquipped ? "equipped" : "failed to equip") << std::endl;
            } else {
            ...
            if (findItemTypeSlot(backpack, "HeadArmor", headArmorRow, headArmorCol)) {
                std::cout << "Equipping head armor from position (" << headArmorRow << "," << (char)('A'+headArmorCol) << ")..." << std::endl;
                bool headEquipped = equipment.equipItemFromBackpack(backpack, headArmorRow, headArmorCol, "HeadArmor");
                std::cout << "Head armor " << (headEquipped ? "equipped" : "failed to equip") << std::endl;
            } else {
            ...
            if (findItemTypeSlot(backpack, "BodyArmor", bodyArmorRow, bodyArmorCol)) {
                std::cout << "Equipping body armor from position (" << bodyArmorRow << "," << (char)('A'+bodyArmorCol) << ")..." << std::endl;
                bool bodyEquipped = equipment.equipItemFromBackpack(backpack, bodyArmorRow, bodyArmorCol, "BodyArmor");
                std::cout << "Body armor " << (bodyEquipped ? "equipped" : "failed to equip") << std::endl;
            } else {
            ...
            if (findItemTypeSlot(backpack, "FootArmor", footArmorRow, footArmorCol)) {
                std::cout << "Equipping foot armor from position (" << footArmorRow << "," << (char)('A'+footArmorCol) << ")..." << std::endl;
                bool footEquipped = equipment.equipItemFromBackpack(backpack, footArmorRow, footArmorCol, "FootArmor");
                std::cout << "Foot armor " << (footEquipped ? "equipped" : "failed to equip") << std::endl;
            } else {
            ...
            if (findItemTypeSlot(backpack, "Pendant", pendantEquipRow, pendantEquipCol)) {
                std::cout << "Equipping pendant from position (" << pendantEquipRow << "," << (char)('A'+pendantEquipCol) << ")..." << std::endl;
                bool pendantEquipped = equipment.equipItemFromBackpack(backpack, pendantEquipRow, pendantEquipCol, "Pendant");
                std::cout << "Pendant " << (pendantEquipped ? "equipped" : "failed to equip") << std::endl;
            } else {
        ...
        try {
            ...
            if (findEmptySlot(backpack, emptyWeaponRow, emptyWeaponCol)) {
                std::cout << "Unequipping weapon to empty position (" << emptyWeaponRow << "," << (char)('A'+emptyWeaponCol) << ")..." << std::endl;
                bool weaponUnequipped = equipment.unequipItemToBackpack(backpack, emptyWeaponRow, emptyWeaponCol, "Weapon");
                std::cout << "Weapon " << (weaponUnequipped ? "unequipped" : "failed to unequip") << std::endl;
            } else {
            ...
            if (findEmptySlot(backpack, secondWeaponRow, secondWeaponCol)) {
                ...
                backpack.addItemAtTile(secondWeaponRow, secondWeaponCol, itemManager.getItem("KAT"), 1);
                ...
                std::cout << "Unequipping weapon to position with same type (" << secondWeaponRow << "," 
                         << (char)('A'+secondWeaponCol) << ")..." << std::endl;
                bool weaponSwapped = equipment.unequipItemToBackpack(backpack, secondWeaponRow, secondWeaponCol, "Weapon");
            ...
            if (findItemTypeSlot(backpack, "Potion", potionRow, potionCol)) {
                ...
                bool armorUnequipped = equipment.unequipItemToBackpack(backpack, potionRow, potionCol, "BodyArmor");
                std::cout << "Body armor " << (armorUnequipped ? "unequipped" : "failed to unequip") << " (should fail)" << std::endl;
            } else {
                // If no potion found, add one to an empty slot
                if (findEmptySlot(backpack, potionRow, potionCol)) {
                    std::cout << "Adding a potion to test unequipping to different type..." << std::endl;
            ...
            if (stackRow != -1) {
                ...
                bool footUnequipped = equipment.unequipItemToBackpack(backpack, stackRow, stackCol, "FootArmor");
                std::cout << "Foot armor " << (footUnequipped ? "unequipped" : "failed to unequip") << " (should fail)" << std::endl;
            } else {
        ...
        try {
            ...
            for (int i = 0; i < 8 && occupiedRow == -1; i++) {
                for (int j = 0; j < 4 && occupiedRow == -1; j++) {
                    try {
                        Item item = backpack.getItemAtTile(i, j);
            ...
            if (occupiedRow != -1) {
                ...
                std::cout << "Trying to add a Helmet to position (" << occupiedRow << "," << (char)('A'+occupiedCol) << ") that has a Weapon..." << std::endl;
