...
int main()
{
    ...
    for (auto& row : parser.GetEquipmentData())
    {
        ...
        if (slot == "WEAPON")
            ...
            eq.setHeadArmor(item);
        else if (slot == "ARMOR_BODY")
            eq.setBodyArmor(item);
        else if (slot == "ARMOR_FOOT")
            eq.setFootArmor(item);
    ...
    Fighter        character(parser.GetUnitStats().at("NAME"), sf::Vector2f(0, 0), nav, true, ctx);
    ...
    std::cout << "Round‑trip complete: wrote to " << outBase << "/" << name << "\n";
    return 0;
}